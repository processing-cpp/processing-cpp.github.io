...
function setup() {
  ...
  cells = Array(cols);
  cellsBuffer = Array(cols);
  for (let x = 0; x < cols; x++) {
    cells[x] = Array(rows).fill(0);
    cellsBuffer[x] = Array(rows).fill(0);
  }
...
function draw() {
  ...
  if (pause && mouseIsPressed) {
    let xCellOver = Math.floor(map(mouseX, 0, width, 0, width / cellSize));
    let yCellOver = Math.floor(map(mouseY, 0, height, 0, height / cellSize));
